...
- Military service is now the foundation of the narrative. Service facts come from Mitchel's 2026-09-23 résumé (89th Airlift Wing; 50+ executive airlift missions across 30+ countries with zero safety-related incidents; onboard with the Vice President and Secretary of State; safety and logistics with flight crews, security teams, and White House staff; chose to pursue AI while serving) plus the station from his earlier résumés. At his request (2026-09-24), Air Force Two and the mission-administration details (in-flight service, baggage, crew hotels and ground transportation, visas, subsistence billing) appear on both the site and the résumé. Nothing is inferred.
...
- Both manuscripts are listed on the site and the résumé at Mitchel's request: the HYDRA results manuscript for Water Resources Research (in preparation) and the NextGen reforecast generation software paper for Environmental Modelling & Software (planned).
...
- The public résumé (`public/mitchelcarson_resume.pdf`, with identical root copies `Resume_2026.pdf` and `Resume_Masters_26.pdf`) is Mitchel's 2026-09-23 design regenerated on 2026-09-24 with Air Force Two, the mission-administration bullet, and the two manuscripts added. The regeneration script reproduced the original glyph for glyph before the additions. Apply the same text to the local JobSearch generator source so the next regeneration keeps them.
